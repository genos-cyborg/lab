...
int currtime=0,gindex=0,wtime[20],totime[20],i,n;
int finish_proc()
{
	for(i=1;i<=n;i++)
	if(proc[i].cpubt!=0)
...
void schedule_proc()
{
	while(!finish_proc())
	{
		...
		proc[pid].inrq=0;
		wtime[pid]+=(currtime-getctime(pid));//waiting time.
		gchart[++gindex].stime=currtime;
		gchart[gindex].pid=pid;
		if(proc[pid].cpubt>=TQ)
		{
			currtime+=TQ;
			proc[pid].cpubt-=TQ;
		}
		...
		if(proc[pid].cpubt==0)
		{
			...
			proc[pid].atime=gchart[gindex].ctime+2;
			totime[pid]+=2;
		}
	}
}
void print_times()
{
	...
	printf("\n AVERAGE WAITING TIME=%.2f",(float)avgw/n);
